...
from langchain_community.document_loaders import (
    ...
    UnstructuredFileLoader,  # Generic loader for other types like TIFF if supported by unstructured
)
from langchain_text_splitters import RecursiveCharacterTextSplitter
from langchain_core.documents import Document
from pydantic import BaseModel, FilePath, field_validator

from core.config import Settings, get_settings, PROJECT_ROOT
...
class DocumentLoaderService:  # Renamed to avoid conflict if RAGService has an inner DocumentLoader
    ...
    def load_document_by_id(self, document_id: str) -> Optional[List[Document]]:
        ...
        # If the file doesn't exist, try different extensions
        if not file_path.exists() or not file_path.is_file():
            # Check for a JSON file with the same base name (from OCR processing)
            base_name = Path(document_id).stem
            json_path = self.processed_docs_path / f"{base_name}.json"

            if json_path.exists():
                logger.info(f"Using JSON file instead: {json_path}")
                file_path = json_path
            else:
                # Try other common extensions
                for ext in [".pdf", ".txt", ".docx"]:
                    alt_path = self.processed_docs_path / f"{base_name}{ext}"
                    if alt_path.exists():
                    ...
                    logger.warning(
                        f"Document not found at path: {file_path} or any alternative paths"
                    )
                    return None
        ...
        try:
            ...
            # Special handling for JSON files from text extraction
            if file_path.suffix.lower() == ".json":
                ...
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    logger.info(
                        f"Loaded and split JSON content from {file_path.name} into {len(split_docs)} chunks."
                    )
                    return split_docs
            else:
