    ...
    async def generate_and_store_speech(
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                logger.info(
                    f"Sending TTS request to {self.settings.COQUI_TTS_URL}/api/tts as FORM DATA with payload: {text[:50]}..."
                )
                # Send as form data instead of JSON
                response = await client.post(
                    f"{self.settings.COQUI_TTS_URL}/api/tts", data=coqui_payload
                )
        ...
        bucket_name = self.settings.MINIO_SPEECH_BUCKET
