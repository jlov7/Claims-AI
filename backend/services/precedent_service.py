...
from models import PrecedentResultItem

logger = logging.getLogger(__name__)

# Global PrecedentService instance for singleton-like behavior
_precedent_service_instance: Optional["PrecedentService"] = None
...
class PrecedentService:
    ...
    def __init__(self, settings_instance: Optional[Settings] = None):
        ...
        # Initialize embedding function with OllamaEmbeddings
        try:
            self.embedding_function = embedding_functions.OllamaEmbeddings(
                model=self.settings.EMBEDDING_MODEL_NAME,
                base_url=self.settings.OLLAMA_BASE_URL,
            )
            logger.info(
                f"PrecedentService initialized OllamaEmbeddings with API base: {self.settings.OLLAMA_BASE_URL} and model: {self.settings.EMBEDDING_MODEL_NAME}"
            )
        except Exception as e:
            logger.error(
    ...
    def find_precedents(
        ...
        try:
            # OllamaEmbeddings expects a single string for embed_query
            query_embedding = self.embedding_function.embed_query(claim_summary)
        except Exception as e:
            logger.error(
