...
# from backend.services.speech_service import settings as speech_settings # REMOVED
...
@pytest.mark.asyncio
async def test_generate_and_store_speech_success(
    service: SpeechService,
):  # Added type hint for clarity
    svc = service
    url, filename = await svc.generate_and_store_speech("text", None, None)
    assert (
        url == f"http://url/{svc.settings.MINIO_SPEECH_BUCKET}/{filename}"
    )  # Changed to use svc.settings
