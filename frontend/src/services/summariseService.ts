...
export const summarise = async (
  ...
  try {
    ...
    const response = await apiClient.post<SummariseResponse>(
      "/summarise",
      request,
    );
    ...
    if (error.response) {
      ...
      if (error.response.data && error.response.data.detail) {
        ...
        errorMessage =
          "The summarise endpoint was not found. Please check server configuration.";
      } else if (error.response.status === 500) {
        errorMessage =
          "Server error processing this document. Please try again or use a different document.";
      }
