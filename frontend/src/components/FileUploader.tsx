...
import { uploadFiles, fetchDocuments } from "../services/uploadService.ts";
...
const FileUploader = () => {
  ...
  // Add a state to track whether we're checking document status
  ...
  const handleUpload = async () => {
    ...
    setIsUploading(true);
    console.log(
      "handleUpload: Initial filesToUpload",
      JSON.stringify(
    ...
    console.log(
      "handleUpload: updatedFilesToUpload (after map, before setFilesToUpload completes)",
      JSON.stringify(
        updatedFilesToUpload.map((f) => ({
          name: f.file.name,
          status: f.status,
        })),
      ),
    ...
    console.log(
      "handleUpload: filesToSubmit (files we intend to send)",
      filesToSubmit.map((f) => f.name),
    );
    ...
    if (filesToSubmit.length === 0) {
      ...
      toast({
        ...
        description: "All files are already uploaded or in an uploading state.",
        status: "info",
        duration: 3000,
      });
    ...
    try {
      console.log(
        "handleUpload: Calling uploadFiles with:",
        filesToSubmit.map((f) => f.name),
      );
      ...
      setFilesToUpload((prev) => {
        const afterUploadUpdate = prev.map((ufs) => {
          const backendResult = response.results.find(
            (res) => res.filename === ufs.file.name,
          );
          if (ufs.status === "uploading" && backendResult) {
            // Check if this ufs was part of the uploaded batch
            const updatedFile = {
              ...ufs,
            ...
            if (backendResult.success && backendResult.document_id) {
              ...
              if (normalizedDocId.endsWith(".json")) {
                ...
                console.log(
                  `Removed .json extension, document_id now: ${normalizedDocId}`,
                );
              }

              // Store the original file extension if applicable
              const originalExt = ufs.file.name.substring(
                ufs.file.name.lastIndexOf("."),
              );
              ...
              if (
                ...
                console.log(
                  `Added original extension, final document_id: ${normalizedDocId}`,
                );
              }
        ...
        console.log(
          "handleUpload: State after processing backend response",
          JSON.stringify(
            afterUploadUpdate.map((f) => ({
              name: f.file.name,
              status: f.status,
              message: f.message,
            })),
      ...
      setFilesToUpload((prev) => {
        const afterErrorUpdate = prev.map((f) => {
          ...
          if (
            filesToSubmit.some(
              (submittedFile) => submittedFile.name === f.file.name,
            ) &&
            f.status === "uploading"
          ) {
            return {
        ...
        console.log(
          "handleUpload: State after catastrophic error",
          JSON.stringify(
            afterErrorUpdate.map((f) => ({
              name: f.file.name,
              status: f.status,
              message: f.message,
            })),
  ...
  // Function to check if documents are ingested
  const checkDocumentStatus = useCallback(async () => {
    ...
    try {
      ...
      if (documents.length > 0) {
        // Update the status of any matching documents
        setFilesToUpload((prev) =>
          prev.map((ufs) => {
            // Find a matching document by name
            const matchingDoc = documents.find(
              (doc) =>
                doc.filename === ufs.file.name ||
                doc.filename === ufs.file.name.replace(/\.[^/.]+$/, "") ||
                doc.id === ufs.file.name ||
                doc.id === ufs.document_id,
            );

            if (
              matchingDoc &&
              matchingDoc.ingested &&
              ufs.status === "success" &&
              !ufs.ingested
            ) {
              console.log(`Document ${ufs.file.name} now shows as ingested`);
              return {
                ...ufs,
                ingested: true,
  ...
  // Check document status periodically after a successful upload
  useEffect(() => {
    const successfulUploads = filesToUpload.some(
      (file) => file.status === "success" && !file.ingested,
    );

    if (successfulUploads && !isUploading && !isCheckingDocuments) {
      // Check immediately after upload
      checkDocumentStatus();

      // Set up a timer to check every 1 second for up to 30 seconds
      let checkCount = 0;
      const maxChecks = 30;
      const interval = setInterval(() => {
        ...
        // Check for any non-ingested successful uploads
        const stillNeedsIngestion = filesToUpload.some(
          (file) => file.status === "success" && !file.ingested,
        );
        if (!stillNeedsIngestion) {
          clearInterval(interval);
          return;
        }
  ...
  }, [filesToUpload, isUploading, isCheckingDocuments, checkDocumentStatus]);
        ...
        >
          ...
          >
            ...
            <Text fontSize="sm" color="gray.500">
              Supported: PDF, TIFF, DOCX (Max {MAX_FILES} files,{" "}
          ...
          >
            {filesToUpload.map((ufs) => (
              ...
              >
                  ...
                  <HStack spacing={3}>
                    ...
                    <VStack align="start" spacing={0}>
                      ...
                      <Text fontSize="xs" color="gray.500">
                        {(ufs.file.size / 1024 / 1024).toFixed(2)} MB
                      </Text>
                    </VStack>
                  </HStack>
                  <HStack spacing={2}>
                    ...
                    <CloseButton
                      size="sm"
                      onClick={() => removeFile(ufs.id)}
                      isDisabled={isUploading && ufs.status === "uploading"}
                    />
                  </HStack>
                </HStack>
                {(ufs.status === "uploading" ||
                ...
                {(ufs.status === "success" || ufs.status === "error") &&
                  ufs.message && (
                    <Text
                      fontSize="xs"
        ...
        {isUploading &&
          ...
          filesToUpload.length > 0 && (
            <Text fontSize="sm" color="gray.500" textAlign="center">
              All selected files are currently uploading or have completed.
            </Text>
          )}
      </CardBody>
    </Card>
