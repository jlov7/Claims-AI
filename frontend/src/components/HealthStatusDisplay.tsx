...
const HealthStatusDisplay: React.FC<HealthStatusDisplayProps> = ({
  health,
  loadingHealth,
}) => {
  return (
    <Box mb={4}>
      ...
      {loadingHealth ? (
        ...
        <Tag
          size="md"
          colorScheme="red"
          title={health?.message || "Check backend services."}
        >
          {health?.status || "Error"}
          {health?.message && health.message.length > 50
            ? `- ${health.message.substring(0, 50)}...`
            : health?.message
              ? `- ${health.message}`
              : "(Check backend logs)"}
        </Tag>
